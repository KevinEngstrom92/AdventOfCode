...
namespace Day08
{
    class Program
    {
        ...
        {
            var puzzleInput = @"gfbd geadcb gaefc agdcf cdgfba dcf abdefc cadbg agfecbd df | acgbd dafcg fd df
...
gbdcea gbdf fgecb fbcdage agefc bge fedbca efgbcd bg debcf | dcgfeb cgaef bcdfe gabfdec";
            ...
            //inputsList.AddRange(splittedInput.Where((x, index) => index % 2 == 0).Select(x => x.Remove(x.Length - 1, 1)));
            foreach (var item in splittedInput)
            {
                var holder = item.Split(" | ");
                inputsList.Add(holder[0]);
                outputList.Add(holder[1]);
            }
            string zero = "";
            string one = "";
            string two = "";
            ...
            foreach (var row in inputsList)
            {
                Console.WriteLine("Running " + currentIndex);
                var temp = inputsList[currentIndex].Split(" ", StringSplitOptions.RemoveEmptyEntries);

                one = temp.Where(x => x.Length == 2).First();
                four = temp.Where(x => x.Length == 4).FirstOrDefault();
                seven = temp.Where(x => x.Length == 3).FirstOrDefault();
                eight = temp.Where(x => x.Length == 7).FirstOrDefault();

                three = temp.Where(x => seven.All(x.Contains) && x.Length == 5).FirstOrDefault();
                nine = temp.Where(x => x.Length == 6 && seven.All(x.Contains) && three.All(x.Contains)).FirstOrDefault();
                five = temp.Where(x => x.Length == 5 && x != three && four.Count(x.Contains) == four.Length-1).FirstOrDefault();
                two = temp.Where(x => x.Length == 5 && x != three && x != five).FirstOrDefault();
                zero = temp.Where(x => eight.Count(x.Contains) == eight.Length - 1 && x.Length == 6 && three.Count(x.Contains) == three.Length -1 && seven.All(x.Contains)).FirstOrDefault();
                six = temp.Where(x => x.Length == 6 && eight.Count(x.Contains) == 6 && !nine.All(x.Contains) && x != zero).FirstOrDefault();
                //outputList.AddRange(splittedInput.Where((x, index) => index % 2 != 0));




                var x = outputList[currentIndex].Split(" ");
#if SILVER
                int[] numbers = new int[4];
                foreach (var item1 in x)
                {
                    if (item1.All(one.Contains) && one.All(item1.Contains))
                    {
                        if (item1.Length == one.Length)
                            numbers[0]++;
                        continue;
                    }
                    if (item1.All(four.Contains) && four.All(item1.Contains))
                    {
                        if (item1.Length == four.Length)
                            numbers[1]++;
                        continue;
                    }
                    if (item1.All(seven.Contains) && seven.All(item1.Contains))
                    {
                        if (item1.Length == seven.Length)
                            numbers[2]++;
                        continue;
                    }
                    if (item1.All(eight.Contains) && eight.All(item1.Contains))
                    {
                        if(item1.Length == eight.Length)
                            numbers[3]++;
                        continue;
                    }
                }
                ...
                foreach (var item1 in x)
                {
                    ...
                    if (item1.All(two.Contains) && two.All(item1.Contains))
                    {
                        if (item1.Length == two.Length)
                            numbers[2]++;
                        num.Add(2);

                        continue;
                    }
                    if (item1.All(three.Contains) && three.All(item1.Contains))
                    {
                        if (item1.Length == three.Length)
                            numbers[3]++;
                        num.Add(3);
                    ...
                    if (item1.All(five.Contains) && five.All(item1.Contains))
                    {
                        if (item1.Length == five.Length)
                            numbers[5]++;
                        num.Add(5);

                        continue;
                    }
                    if (item1.All(six.Contains) && six.All(item1.Contains))
                    {
                        if (item1.Length == six.Length)
                            numbers[6]++;
                        num.Add(6);
                    ...
                    if (item1.All(nine.Contains) && nine.All(item1.Contains))
                    {
                        if (item1.Length == nine.Length)
                            numbers[9]++;
                        num.Add(9);

                        continue;
                    }
                    if (item1.All(zero.Contains) && zero.All(item1.Contains))
                    {
                        if (item1.Length == zero.Length)
                            numbers[0]++;
                        num.Add(0);
